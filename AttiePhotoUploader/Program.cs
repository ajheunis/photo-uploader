...
const string galleryName = "emma-graduation-party";
...
{
    foreach (var file in Directory.EnumerateFiles(thumbnailsPath, "*.*",
...
foreach (var file in Directory.EnumerateFiles(imagesPath, "*.png",
    ...
    string.IsNullOrEmpty(containerName))
{
    throw new InvalidOperationException(
        "Azure Storage connection string or container name is not set in the user secrets.");
}
...
static async Task UploadFolderToBlobStorage(string folderPath,
    BlobContainerClient containerClient, string galleryName)
{
    // Delete all blobs in the container that start with 'galleries' and the gallery name
    var blobsToDelete = containerClient
        .GetBlobs(prefix: $"galleries/{galleryName}").Select(b => b.Name)
        .ToList();
                 ...
                 SearchOption.AllDirectories))
    {
        ...
        var relativePath =
            Path.GetRelativePath(imagesPath, file).Replace("\\", "/");
